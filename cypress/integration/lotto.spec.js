...
describe('기능 테스트', () => {
  ...
  it('수동 구매를 선택한 경우 직접 로또 번호를 입력할 수 있다.', () => {
    ...
    cy.get('#cost-submit-button').click();
    cy.get('#manual-purchase-button').click();
    cy.get('.lotto-number').eq(0).type(1);
    cy.get('.lotto-number').eq(1).type(2);
    cy.get('.lotto-number').eq(2).type(3);
    cy.get('.lotto-number').eq(3).type(4);
    cy.get('.lotto-number').eq(4).type(5);
    cy.get('.lotto-number').eq(5).type(6);
  ...
  it('소비자가 받은 각각의 복권에서 중복되는 숫자가 존재하면 안된다.', () => {
    ...
    cy.get('#cost-submit-button').click();
    cy.get('#auto-purchase-button').click();
    cy.get('#auto-count-input').type(5);
    cy.get('#auto-count-submit-button').click();
...
describe('유저 입력 값 테스트', () => {
  ...
  it('입력된 로또번호 중 1 ~ 45 사이의 숫자가 아닌 숫자가 있다면 안내메시지를 출력한다.', () => {
    ...
    cy.get('.lotto-number').eq(0).type(1);
    cy.get('.lotto-number').eq(1).type(2);
    cy.get('.lotto-number').eq(2).type(46);
    cy.get('.lotto-number').eq(3).click();
    cy.get('@alertStub').should('be.calledWith', MESSAGE.NUMBER_RANGE_EXCEEDED);
    cy.get('.lotto-number').eq(2).should('have.text', '');
  });
  ...
  it('입력된 로또번호 중 중복된 번호가 있다면 안내메시지를 출력한다.', () => {
    ...
    cy.get('.lotto-number').eq(0).type(1);
    cy.get('.lotto-number').eq(1).type(2);
    cy.get('.lotto-number').eq(2).type(2);
    cy.get('.lotto-number').eq(3).click();
    cy.get('@alertStub').should(
      'be.calledWith',
  ...
  it('로또번호가 모두 입력되지 않으면 안내메시지를 출력한다.', () => {
    ...
    cy.get('@alertStub').should(
      'be.calledWith',
      MESSAGE.SHOULD_FILL_ALL_LOTTO_NUMBERS,
    );
  });
  ...
  it('당첨번호가 모두 입력되지 않으면 결과를 확인할 수 없다.', () => {
    ...
    cy.get('@alertStub').should(
      'be.calledWith',
      MESSAGE.SHOULD_FILL_ALL_WINNING_NUMBERS,
    );
    cy.get('#modal').should('not.be.visible');
  });
});
