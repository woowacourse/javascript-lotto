import {
  isOverRangeBonusNumber,
  isOverRangeWinningNumbers,
  isOverlapped,
  isInvalidCount,
...
describe('지난주 당첨 번호는 중복되지 않는 1 이상 45 이하의 6개의 숫자여야 한다.', () => {
  test('지난주 당첨 번호의 숫자들은 모두 1 이상의 숫자여야 한다.', () => {
    ...
    expect(isOverRangeWinningNumbers(winningNumbers)).toBeFalsy();
  });
  ...
  test('지난주 당첨 번호애는 1 미만의 숫자가 있으면 안된다.', () => {
    ...
    expect(isOverRangeWinningNumbers(winningNumbers)).toBeTruthy();
  });
...
describe('보너스 당첨 번호는 지난주 당첨 번호에 속해있지 않는 1 이상 ~ 45 이하의 숫자여야 한다.', () => {
  ...
  test('보너스 당첨 번호는 1 이상의 숫자여야 한다.', () => {
    const bonusNumber = 1;
    expect(isOverRangeBonusNumber(bonusNumber)).toBeFalsy();
  });
  ...
  test('보너스 당첨 번호는 1 미만의 숫자이면 안 된다.', () => {
    const bonusNumber = 0;
    expect(isOverRangeBonusNumber(bonusNumber)).toBeTruthy();
  });
