...
describe('구입 금액 검증 테스트 ', () => {
  ...
  test('입력 값이 빈 칸이 아니어야 한다.', () => {
    const cashInput = '';
    expect(() => lottoManger.buyLotto(cashInput)).toThrow(ERROR_MESSAGE.EMPTY_INPUT);
  });
  test('입력 값의 범위가 1000 - 50000인지 검증한다.', () => {
    const cashInput = '55000';
    expect(() => lottoManger.buyLotto(cashInput)).toThrow(ERROR_MESSAGE.OUT_OF_RANGE);
  });
  test('입력 값이 1000 단위인지 검증한다.', () => {
    const cashInput = '1500';
    expect(() => lottoManger.buyLotto(cashInput)).toThrow(ERROR_MESSAGE.INVALID_UNIT);
  });
