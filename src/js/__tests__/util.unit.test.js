...
describe('당첨번호 유효성 검사 유틸 테스트', () => {
  test('번호간 중복이 없어야 한다', () => {
    ...
    expect(isNotDuplicateNumberExist(lottoWinningNumbers)).toBe(true);
  });
  ...
  test('각 번호는 1이상 45이하의 숫자들로만 이루어져야한다.', () => {
    ...
    expect(isAllNumberInRange(lottoWinningNumbers)).toBe(true);
  });
...
describe('보너스번호 유효성 검사 유틸 테스트', () => {
  test('1이상 45이하의 숫자여야 한다.', () => {
    ...
    expect(isNumberInRange(lottoWinningBonusNumber)).toBe(true);
  });
