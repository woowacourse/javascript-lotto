...
describe('[케이스 3] 당첨 결과를 확인할 수 있어야 한다.', () => {
  // given
  const lottoVendor = new LottoVendor();
  lottoVendor.lottos = [
    { numbers: [1, 2, 25, 27, 38, 39] },
    { numbers: [21, 23, 35, 40, 44, 45] },
    { numbers: [1, 5, 8, 15, 33, 45] },
  ];
  lottoVendor.paidMoney = 3000;

  const lottoResult = new LottoResult(lottoVendor);

  test('당첨번호와 구입한 로또 번호를 비교하여 당첨 금액별 당첨 번호 개수를 계산할 수 있어야 한다.', () => {
    lottoResult.winningNumbers = [1, 2, 25, 35, 40, 45];
    lottoResult.bonusNumber = 5;
    lottoResult.calculateWinningCounts();
  ...
  test('구입 금액과 당첨된 금액을 비교하여 수익률을 계산할 수 있어야 한다.', () => {
    ...
    expect(lottoResult.lottoYield).toBe(333);
  });
});
