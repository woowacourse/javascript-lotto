...
import { isUnderMinimum, isNotThousandUnit } from "../utils/general.js";

/* eslint-disable no-undef */
describe("구입할 금액 유효성 테스트", () => {
  test("1000원 이상 이어야 한다.", () => {
    const amount = 1000;
    expect(isUnderMinimum(amount)).toBeFalsy();
  });
  ...
  test("1000원 단위로 입력 가능하다.", () => {
    const amount = 2000;
    expect(isNotThousandUnit(amount, AMOUNT.UNIT)).toBeFalsy();
  });
