  ...
  describe("입력받은 갯수 유효성검사 테스트", () => {
    test.each(["1", true, {}, [], undefined, null])(
      ...
      (lottoCount) => {
        const lottoStore = new LottoStore();
        expect(() => {
          lottoStore.generateLottosNumbers(lottoCount);
        }).toThrow();
      },
    );
...
describe("입력받은 배열의 길이만큼 로또를 발행한다.", () => {
  ...
  test("입력받은 배열의 길이만큼 로또 인스턴스를 생성한다.", () => {
    ...
    const lottoes = lottoStore.issueLottos(sixNumbersArray);

    lottoes.forEach((lotto) => expect(lotto).toBeInstanceOf(Lotto));
  });
});
