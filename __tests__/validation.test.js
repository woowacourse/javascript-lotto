import { validateMoney, validateLottoNumber, validateBonus } from "../src/domain/validation.js";
...
describe('로또 구입 금액',() => {
  ...
  test('로또 구입 금액은 1,000원으로 나누어 떨어지지 않으면 에러를 발생한다.',() => {
    ...
    expect(() => {
        validateMoney(money);
    }).toThrow(ERROR_MESSAGES.MONEY.REST_VALUE);
  });
  ...
  test.each([
    ...
    expect(() => {
        validateMoney(money);
    }).toThrow(ERROR_MESSAGES.MONEY.EMPTY_VALUE);
  });
  ...
  test('로또 구입 금액은 100,000원 이하만 가능하다.',() => {
    ...
    expect(() => {
        validateMoney(money);
    }).toThrow(ERROR_MESSAGES.MONEY.MAX_VALUE);
  });
})
...
describe('로또 숫자',() => {
  test.each([
    ...
    expect(() => validateLottoNumber(lottoNumbers))
      .toThrow(ERROR_MESSAGES.LOTTO.NUMBER.QUANTITY);
  });
  ...
  test.each([
    ...
    expect(() => validateLottoNumber(lottoNumbers))
      .toThrow(ERROR_MESSAGES.LOTTO.NUMBER.RANGE);
  });
  ...
  test.each([
    ...
    expect(() => validateLottoNumber(lottoNumbers))
      .toThrow(ERROR_MESSAGES.LOTTO.NUMBER.DUPLICATION);
  });
});
...
describe('보너스 숫자', () => {
  ...
  test.each([
    ...
    expect(() => validateBonus(bonus, winningLotto))
      .toThrow(ERROR_MESSAGES.LOTTO.BONUS.RANGE);
  });

  test('보너스 번호는 당첨 로또에 있는 숫자와 중복되면 안된다.', () => {
    const bonus = 6;
    expect(() => validateBonus(bonus, winningLotto))
      .toThrow(ERROR_MESSAGES.LOTTO.BONUS.DUPLICATION);
  });
});
