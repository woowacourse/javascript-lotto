...
describe('구입 금액 검증 테스트', () => {
  describe('정상 케이스', () => {
    test(`구입 금액은 숫자여야 하고, ${PURCHASE_PRICE.UNIT.toLocaleString()}원 단위이며, ${PURCHASE_PRICE.MIN.toLocaleString()}~${PURCHASE_PRICE.MAX.toLocaleString()} 사이에 있어야 한다.`, () => {
  ...
  describe('예외 케이스', () => {
    ...
    test.each([999, 1000001, -Infinity, Infinity])(
      `구입 금액이 ${PURCHASE_PRICE.MIN.toLocaleString()}원 이상 ${PURCHASE_PRICE.MAX.toLocaleString()} 이하가 아니면 에러가 발생한다. (입력 값 : %p)`,
      (purchasePrice) => {
        expect(() =>
          validateRange({
    ...
    test.each([1001, 1999, 3023, 999999])(
      `구입 금액이 ${PURCHASE_PRICE.UNIT.toLocaleString()}원 단위로 나누어 떨어지지 않으면 에러가 발생한다. (입력 값 : %p)`,
      (purchasePrice) => {
