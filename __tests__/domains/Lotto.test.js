...
describe('로또 클래스 테스트', () => {
  ...
  describe('예외 케이스', () => {
    test('로또 번호가 숫자가 아니면 에러가 발생한다.', () => {
      ...
      expect(() => new Lotto(numbers)).toThrow(ERROR_MESSAGE.LOTTO.INVALID_TYPE);
    });

    test('로또 번호가 6개가 아니면 에러가 발생한다.', () => {
      const numbers = [1, 2, 3, 4, 5];
      expect(() => new Lotto(numbers)).toThrow(ERROR_MESSAGE.LOTTO.INVALID_LENGTH);
    });

    test('로또 번호의 범위가 1~45 사이가 아니면 에러가 발생한다.', () => {
      const numbers = [0, 2, 3, 4, 5, 46];
      expect(() => new Lotto(numbers)).toThrow(ERROR_MESSAGE.LOTTO.INVALID_RANGE);
    });
