import parseAndValidateWinningNumbers from "../src/domain/processors/parseAndValidateWinningNumbers";

describe("당첨 번호 관련 도메인 검증", () => {
  test("숫자와 구분자(,)로 이루어진 값이 아니라면 에러가 발생한다.", () => {
    const winningNumbers = "가나다";
    expect(() => parseAndValidateWinningNumbers(winningNumbers)).toThrow();
  });
  ...
  test("당첨번호가 들어오면 당첨번호 배열이 반환된다.", () => {
    ...
    expect(parseAndValidateWinningNumbers(winningNumbers)).toEqual(expectWinningNumbers);
  });
});
