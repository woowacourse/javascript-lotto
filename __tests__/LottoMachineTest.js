...
describe("로또 머신 테스트", () => {
  ...
  test("구입 개수 만큼 로또 생성 테스트", () => {
    ...
    expect(lottoMachine.getLottos()[0].getLottoNumber()).toEqual([
      1, 4, 5, 6, 8, 9,
    ]);
    expect(lottoMachine.getLottos()[1].getLottoNumber()).toEqual([
      3, 17, 19, 21, 23, 31,
    ]);
    expect(lottoMachine.getLottos()[2].getLottoNumber()).toEqual([
      35, 37, 39, 41, 42, 45,
    ]);
  });
  ...
  test("수익률 계산 로직 테스트", () => {
    pickNumberInRange
      .mockReturnValueOnce([1, 2, 3, 10, 11, 12])
      .mockReturnValue([10, 11, 12, 13, 14, 15]);
