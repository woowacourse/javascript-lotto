import LottoPurchasePriceValidator from '../src/validators/LottoPurchasePriceValidator';
...
describe('구입 금액 테스트', () => {
  test('구입 금액은 1000원 단위로 나누어 떨어지지 않으면 에러를 발생한다.', () => {
    expect(() => {
      LottoPurchasePriceValidator.validate(1001);
    }).toThrow(ERROR_MESSAGES.invalidDividedUnit);
  });

  test('구입 금액은 1000원 단위로 나누어 떨어져야 한다.', () => {
    expect(() => {
      LottoPurchasePriceValidator.validateDividedUnit(1000);
...
describe('당첨 번호 입력 테스트', () => {
  ...
  test('입력된 당첨 번호가 1~45 사이의 숫자가 아닐 경우 에러를 발생한다.', () => {
    expect(() => {
      LottoValidator.validateWinningNumbersRange([0, 2, 3, 4, 5, 6]);
