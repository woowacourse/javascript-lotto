...
describe("WinningNumbersAndBonusNumberBuilder 클래스 유닛 테스트", () => {
  describe("validateWinningNumbers", () => {
    ...
    test("당첨 번호가 정수가 아니면 에러를 반환한다.", () => {
      ...
      // when & then
      expect(() =>
        builder.validateWinningNumbers([1, 2, 3, 4, 5, 5.5]),
      ).toThrow();
    });
    ...
    test("당첨 번호가 6개가 아니면 에러를 반환한다.", () => {
      ...
      // when & then
      expect(() => builder.validateWinningNumbers([1, 2, 3, 4, 5])).toThrow();
    });

    test("당첨 번호가 1부터 45까지의 범위를 벗어나면 에러를 반환한다.", () => {
      // given
      const builder = new WinningNumbersAndBonusNumberBuilder();
  ...
  describe("validateBonusNumber", () => {
    ...
    test("당첨 번호가 정수가 아니면 에러를 반환한다.", () => {
      ...
      // then
      expect(() => builder.validateBonusNumber(6.1)).toThrow();
    });
