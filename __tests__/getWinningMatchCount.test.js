...
describe('로또 당첨 개수 테스트', () => {
  ...
  test('로또 당첨 개수가 5개여야 한다.', () => {
    const lottos = [[8, 12, 14, 23, 41, 1]];
    const matchedCount = getWinningMatchCount(lottos, { winningNumbers, bonusNumber });

    expect(matchedCount).toEqual([0, 0, 0, 0, 0, 1, 0, 0]);
  });

  test('로또 당첨 개수가 5개이고 보너스 넘버가 있어야 한다.', () => {
    const lottos = [[8, 12, 14, 23, 41, 45]];
    const matchedCount = getWinningMatchCount(lottos, { winningNumbers, bonusNumber });
