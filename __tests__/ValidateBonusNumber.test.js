...
describe("보너스 번호 유효성 검사", () => {
  ...
  test("1~45 사이 숫자가 아닌 경우 에러 발생", () => {
    ...
    expect(() => validateBonusNumber(lottoNumbers, inputBonusNumber)).toThrow(BONUS_NUMBER_ERROR_MESSAGE.RANGE);
  });
  test("보너스 번호가 로또 번호와 중복되는 경우 에러 발생", () => {
    const bonusNumberInput = "3";

    expect(() => validateBonusNumber(lottoNumbers, bonusNumberInput)).toThrow(BONUS_NUMBER_ERROR_MESSAGE.DUPLICATE);
  });
});
