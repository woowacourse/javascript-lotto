...
- [ ] 구입 금액 입력받기
  ...
  3. Then:
     - 금액이 초기화 된다.
     - 실패케이스
       - 음수 또는 소수인 경우 => alert로 에러메세지를 표시한다. input에 다시 포커스가 맞춰지고, 입력한 금액은 초기화된다.
     - 성공케이스: 입력된 금액으로 살 수 있는 로또 장수 만큼에 대한 로또 번호 입력칸이 표시된다.

- [ ] 로또 추가 구매 기능

  1. Given: 초기화면에 구입 입력 Form과 로또 번호 입력칸이 보여진다
  2. When: 금액을 입력한 후 확인 버튼을 클릭한다 (로또 1장의 가격은 1,000원이다.)
  3. Then:
     - 금액이 초기화 된다.
     - 실패케이스
       - 음수 또는 소수인 경우 => alert로 에러메세지를 표시한다. input에 다시 포커스가 맞춰지고, 입력한 금액은 초기화된다.
     - 성공케이스: 입력된 금액으로 살 수 있는 로또 장수 만큼에 대한 로또 번호 입력칸이 기존의 로또 번호 입력칸의 하단에 추가로 표시된다.
       - 총 구매가능한 로또의 장수는 100장이다.

- [ ] 로또 번호 입력 기능(수동 구매 기능)

  1. Given: 유저가 입력한 금액에 해당하는 로또 장수만큼의 '자동・수동 구분란과 번호 입력 input 태그 6개'가 표시되어 있다.

     - Default값은 자동이며, 번호 입력 input 태그는 모두 disable 되어 있다

  2. When: 수동 구매 버튼을 클릭하고 번호 여섯개를 입력한다
  3. Then:
     - disable 되어 있던 번호 입력 input태그가 모두 enable하게 된다.
     - 실패케이스: 각 번호 입력시 [1, 45] 정수가 입력되지 않은 경우, 유저에게 바로 에러메세지를 보낸다.
     - 성공케이스: 모든 input에 유효한 입력이 입력되어, 우측 하단의 발급 버튼을 클릭하면 발급된 로또 장수만큼 로또 아이콘이 표시된다.

- [ ] 번호보기 토글

  1. Given: 유저가 구입할 금액에 해당하는 로또 장수만큼이 발급되어 로또 아이콘으로 표시되어 있다. 번호보기 토글은 비활성화되어 있다.
  2. When: 비활성화된 번호보기 토글을 클릭한다.
  3. Then

     - 번호보기 토글이 파란색으로 활성화된다.
     - 각 로또 장수에 대한 숫자 6개 및 자동발급・수동발급 여부가 표시된다.
...
- [ ] 나머지 표시기능: 입력한 금액(cash)에 해당하는 로또 장수를 구매한 후 남은 나머지를 표시한다.
- [ ] 웹 접근성
- [ ] 시멘틱 태그 적용
- [ ] UX를 향상시키는 실시간 input validation
- [ ] state가 변화되었는지 확인하는 로직의 간결화
