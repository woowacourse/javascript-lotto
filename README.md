...
- 정상적인 로또 구입금액을 제출하면 수동번호선택을 위한 로또용지 추가 버튼과 로또 발행 버튼이 표시된다.
  - 전체 자동구매를 기본값으로 한다.
- 수동번호선택 추가 버튼을 누를 때마다 로또번호 선택용지가 한 장씩 추가된다.
  - 각 로또번호 선택용지는 적용수량을 조정할 수 있다. 적용수량의 최대값은 남은 로또 장수이다.
  - 적용수량이 구입한 금액을 넘어서면 추가 버튼을 눌러도 수동구매 추가 버튼을 눌러도 로또번호 선택용지가 추가되지 않는다.
  - 수동구매 삭제 버튼을 누르면 수동구매 번호 입력란이 사라진다.
- 로또복권 발행 버튼을 누르면 로또를 발행을 진행한다.
  - 수동구매하고 남은 금액이 있을 경우 자동구매 옵션으로 구매된다는 안내메세지를 Confirm으로 표시한다.
