...
- 정상적인 로또 구입금액을 투입하면 로또 발급 단계로 넘어갈 수 있다.
  - [ ] 로또 구입금액 투입 후 로또 구입금액 입력버튼은 비활성화된다.
...
- 로또용지를 추가해서 수동구매를 할 수 있다.
  ...
  - [ ] 로또용지에서 6개보다 적게 고르면 상태메세지가 화면에 표시되고 로또 발급버튼이 비활성화 된다.
...
- 로또복권 발급 버튼을 누르면 로또를 발급을 진행한다.
  - [ ] 자동/수동구매 수량이 적힌 안내메세지를 Confirm으로 표시한다.
  - [ ] '확인'을 선택할 경우, 로또를 발급해서 화면에 표시한다.
  - [ ] 더 이상 필요하지 않은 화면에서 로또용지와 로또용지 추가버튼을 숨기고, 로또발급하기 버튼을 비활성화한다.
