...
- [ ] 사용자가 구매한 로또의 개수를 보여주고, 개수만큼 티켓 이모지를 가로로 보여준다.
  - [ ] 구매한 번호는 가려져 있는 모습을 defaut로 한다.
- [ ] '번호 보기'를 눌렀을 때, 각 로또의 번호를 보여준다.
  - [ ] 로또의 각 번호는 중복이 존재하지 않는다.
- [ ] 지난주 당첨 번호 입력 폼을 보여준다. (기능 구현 X)
...
- 예외상황
  - 금액 입력 예외상황
    - [ ] 사용자가 금액을 1,000원 단위로 입력하지 않은 경우, alert로 알린다.
