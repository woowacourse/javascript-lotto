...
- [x] 사용자는 로또 구입 금액을 입력한다.
  ...
  - [x] 로또 구입 금액은 최소 1,000원, 최대 100,000원으로 제한한다.
  - [x] 로또 구입 금액을 잘못 입력했을 경우, alert로 사용자에게 알린다.
- [x] 사용자는 로또 혼합 구매, 전체 자동 구매 버튼을 선택할 수 있다.
  - [x] 혼합 구매를 선택하면, 수동 입력폼이 주어진다. (로또 티켓 1개의 입력 폼)
  - [x] 저장을 누르면, 한개의 티켓 이모지가 추가되고 번호를 확인할 수 있다.
  - [x] 저장 이후에 로또 번호의 수정은 불가능하다.
  - [x] '완료(나머지 자동 구매)' 버튼을 선택하면, 남은 양의 로또가 모두 자동 구매 된다.
- [x] 사용자가 구매한 로또의 개수를 보여주고, 개수만큼 티켓 이모지를 가로로 보여준다.
  - [x] 구매한 번호는 가려져 있는 모습을 default로 한다.
- [x] '번호 보기'를 눌렀을 때, 각 로또의 번호를 보여준다.
  - [x] 로또의 각 번호는 중복이 존재하지 않는다.
...
- 정상 로직
  ...
  - [x] 사용자가 구입금액을 입력하면, '혼합 구매', '자동 구매' 버튼이 보인다.
  - [x] '자동 구매'를 선택하면, 전체 자동 구매가 이루어진다.
  - [x] '혼합 구매'를 선택하면, 수동 입력폼이 보인다.
  - [x] 저장을 누르면, 티켓 이모지가 추가되고 번호를 확인할 수 있다.
  - [x] '완료(나머지 자동 구매)' 버튼을 선택하면, 나머지 양의 로또가 모두 자동구매 된다.

- 예외상황
  - [x] 수동 구매를 할 때, 로또 번호는 중복이 있을 수 없다.
