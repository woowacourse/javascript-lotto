...
- [ ] 구입할 금액 입력 후, 로또를 수동 구매하면 구입한 로또 목록에 해당 로또가 추가된다.
- [ ] 구입할 수 있는 로또 개수의 개수가 0개이면 수동 구매 폼과 자동 구매 버튼이 비활성화된다.
- [ ] 나머지 자동 구매 버튼을 클릭하면 구입할 수 있는 로또의 개수만큼 자동 구매되어 구입한 로또 목록에 추가된다.

#### 예외사항(테스트)

- [ ] 입력된 수동 구매 번호가 1 ~ 45 사이의 숫자가 아니거나, 중복되면
