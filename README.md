...
- [x] `figma` 시안에 따라 웹 `UI` 구현 및 `UI`와 `Domain` 연동
  - [x] 구입 금액 입력 창 구현
  - [x] `구입` 버튼 클릭 시 발행된 로또 화면에 출력
  - [x] 로또 발행된 뒤에 당첨 번호와 보너스 번호 입력 받는 부분 화면에 출력
  - [x] 결과 확인 버튼 클릭 시 화면에 당첨 통계 결과 출력 (모달창으로 구현)
  - [x] 모달창에서 `X`버튼과 `다시 시작하기`버튼 클릭 시 서로 다른 화면 출력
    - [x] `X`버튼: 현재 결과 내용 그대로 보여주기 (구입 금액과 당첨번호, 보너스 번호 입력된 그대로)
    - [x] `다시 시작하기`버튼: 리렌더링 후 다시 초기 상태, 즉 구입 금액 입력 창만 있는 상태로 돌아가기
  - [x] 유효하지 않은 입력에 대한 에러 메시지 화면에 출력 (구입금액, 당첨번호, 보너스번호)
